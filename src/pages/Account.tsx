...
const Account = () => {
  ...
  const loadPickupOrders = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from('pickup_orders').select('id, waste_type, waste_subtype, weight, status, created_at, reward_amount').eq('user_id', user.id).order('created_at', {
        ascending: false
      }).limit(5);
      if (error) {
  ...
  const calculateTotalCash = () => {
    let total = pickupOrders.filter(order => order.status === 'completed' && order.reward_amount).reduce((sum, order) => sum + order.reward_amount, 0);
  ...
  const getRewardValue = (rewardType: string, rewardData: RewardData) => {
    switch (rewardType) {
      case 'cash':
        return <div className="text-2xl font-bold text-green-600 flex items-center justify-center">
            <IndianRupee size={20} className="mr-1" />
            {rewardData.cashReward.toFixed(2)}
          </div>;
      case 'ecoscore':
        return <div className="text-2xl font-bold text-green-600">
  ...
  return <div className="min-h-screen flex flex-col bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 text-gray-200">
      <Navbar />
      <main style={{
        ...
        <div className="max-w-6xl mx-auto">
          <div className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between">
            ...
            <div className="mt-4 md:mt-0 flex gap-2">
              <Button variant="outline" onClick={() => setEditMode(!editMode)} className="bg-white text-gray-800">
          ...
          <Tabs defaultValue="profile">
            ...
            <TabsContent value="rewards" className="mt-6">
              <div className="space-y-6">
                ...
                {rewardData && <Card className="transition-colors hover:bg-white border-2 border-green-200 bg-green-50">
                    <CardHeader>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="text-center p-4 bg-white rounded-lg border">
                          ...
                          <div className="text-sm text-gray-600 mt-1">
                            {rewardData.selectedRewardType === 'cash' ? 'Cash Reward' : rewardData.selectedRewardType === 'ecoscore' ? 'Eco Points' : 'Gift Card Value'}
                          </div>
                        </div>
                        <div className="text-center p-4 bg-white rounded-lg border">
                ...
                <Card className="transition-colors hover:bg-white">
                  ...
                  <CardContent>
                    ...
                    <div className="space-y-4">
                      {pickupOrders.filter(order => order.status === 'completed').slice(0, 3).map((order, i) => <div key={order.id} className="flex justify-between items-center p-4 border rounded-lg transition-colors hover:bg-white">
                          <div>
                      ...
                      {pickupOrders.filter(order => order.status === 'completed').length === 0 && <p className="text-gray-500 text-center py-4">No completed orders yet.</p>}
                    </div>
                  </CardContent>
                  <CardFooter>
