...
const Reward = () => {
  ...
  useEffect(() => {
    // Get reward data from localStorage
    const storedRewardData = localStorage.getItem('rewardData');
    if (storedRewardData) {
      setRewardData(JSON.parse(storedRewardData));
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updateData = {
        user_id: user.id,
        total_cash_earned: (existingData?.total_cash_earned || 0) + (selectedReward === 'cash' || selectedReward === 'egift' ? rewardData.cashReward : 0),
        total_eco_points: (existingData?.total_eco_points || 0) + (selectedReward === 'ecoscore' ? rewardData.ecoPoints : 0),
        total_orders: (existingData?.total_orders || 0) + 1
      };
      ...
      // Clear the reward data from localStorage
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <Card className="shadow-lg">
            <CardHeader>
              ...
              <CardDescription>
                Based on your waste type and quantity ({rewardData.weight}kg), you qualify for one of these rewards
              </CardDescription>
            </CardHeader>
              ...
              <CardContent>
                <RadioGroup value={selectedReward} onValueChange={setSelectedReward} className="space-y-4">
                  ...
                  <div className="flex items-center space-x-2 border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition-colors">
                    ...
                    <div className="text-green-600 font-bold">{rewardData.ecoPoints} pts</div>
                  </div>
                  ...
                  <div className="flex items-center space-x-2 border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition-colors">
                    ...
                    <div className="text-green-600 font-bold flex items-center">
                      <IndianRupee className="h-4 w-4 mr-1" />
                      {(rewardData.cashReward * 1.1).toFixed(2)}
                    </div>
                  </div>
                </RadioGroup>
              </CardContent>
